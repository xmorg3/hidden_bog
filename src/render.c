...
{
  if( left >=1000 ) {
  	SDL_RenderCopy(gc->renderer, get_texture_by_objectid(gc,left), vp0_object_left, &gc->player_viewport);
  }
  if( right >=1000 ) {
  	SDL_RenderCopy(gc->renderer, get_texture_by_objectid(gc,right), vp0_object_right, &gc->player_viewport);
  }
