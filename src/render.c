...
{//gc->wall_all
  ...
  if( left == 2) {
    ...
    drawto_viewport(gc, gc->wall_left_fov2_blank); 
  }
  if ( left == 3 ) {
    drawto_viewport(gc, gc->tall_wall_left_fov2);
  }
  if( left == 4) {
    drawto_viewport(gc, gc->wall_left_fov2_blank);
  ...
  if( right == 4) {
    drawto_viewport(gc,gc->wall_right_fov2_blank);
  ...
  if( middle==4) {
    drawto_viewport(gc,gc->wall_front_fov1_blank);
...
void draw_fov1(GameCore *gc, int left, int middle, int right)
{
  ...
  if( left==2 )  {
    ...
    drawto_viewport(gc, gc->wall_left_fov1_blank);
  }
  if( left ==3)  {
    //SDL_RenderCopy(gc->renderer, gc->tall_wall_left_fov1, NULL, &gc->player_viewport);
    drawto_viewport(gc, gc->tall_wall_left_fov1);
  }
  if( left == 4)  {
    drawto_viewport(gc,gc->wall_left_fov1_blank);
...
void draw_fov0(GameCore *gc, int left, int middle, int right)
{
  ...
  if( left==2)  {
    ...
    drawto_viewport(gc, gc->wall_left_fov0_blank);
  }
  if( left ==3)  {
    //SDL_RenderCopy(gc->renderer, gc->tall_wall_left_fov0, NULL, &gc->player_viewport);
    drawto_viewport(gc, gc->tall_wall_left_fov0);
  }
  if( left == 4)  {
    drawto_viewport(gc, gc->wall_left_fov0_blank);
  ...
  if( right ==3 ) {
    drawto_viewport(gc, gc->tall_wall_right_fov0);
  }
  if( right == 4)  {
