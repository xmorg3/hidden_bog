...
{//int screen_size_x, screen_size_y;
  ...
  gc->char_frame_rect = fast_rect(360, 10, 100, 100 );
...
void draw_character_1attribute(GameCore *gc, int x, int y, int stat, char *str)
{
  ...
  SDL_RenderCopy(gc->renderer, sdl_printf_font(gc, gc->font0, str), NULL, gc->c_text_size);
  sdl_set_textpos(gc,x+130,y);
  sprintf(stat_text, "%2d", stat);
  SDL_RenderCopy(gc->renderer, sdl_printf_font(gc, gc->font0, stat_text), NULL, gc->c_text_size);
}
void draw_minusplus_buttons(GameCore *gc, int x, int y)
{
  SDL_Rect dst, dst2;
